...
def burst_align(reference_image_index, raw_images, parallel=False):
    ...
    for level in range(pyramid_levels):
        ...
        if parallel:
            motion_matrix = parallel_align_level_images(level, np.array(pyramid_images[level]), motion_matrix, reference_image_index, block_size=8 if level < 2 else 16, search_window=4)
        else:
...
@njit(parallel=True)
def parallel_align_level_images(level, pyramid_level_images, motion_matrix, reference_image_index, block_size=16, search_window=4):
    level_reference_image = pyramid_level_images[reference_image_index]
    x_max, y_max = level_reference_image.shape

    for i in prange(motion_matrix.shape[0]):
        for x_ptr in prange(0, x_max//block_size + 1):
            for y_ptr in prange(0, y_max // block_size + 1):
                x = x_ptr * block_size
                y = y_ptr * block_size
                ...
                best_match = np.zeros(3, dtype=np.int64)
                best_match_error = np.inf
...
def parallel_align(motion_matrix, raw_images, block_size=16):
    print(motion_matrix.shape, raw_images.shape)
    num_images = raw_images.shape[0]
    aligned_burst_patches = [None] * num_images

    def align_image(i):
        """Helper function to align a single image"""
        ...
        aligned_burst_patches[i] = aligned_image_patches
            
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(align_image, range(num_images))
...
def parallel_sparse_burst_align(reference_image_index, raw_images):
    """Pyramid block matching algorithm"""
    pyramid_levels = 4
    downsample_factor = 4
    ...
    pyramid_images = pyramid_images[::-1]
    pyramid_shapes = [image[0].shape for image in pyramid_images]
    motion_matrix_shapes = []
    # print("pyramid shapes: ", pyramid_shapes)

    for level in range(pyramid_levels):
        block_size = 8 if level < 2 else 16
        motion_matrix_shape = np.array(pyramid_shapes[level])//block_size
        motion_matrix_shapes.append(motion_matrix_shape)

    # Initialize the motion vectors to zeros initially
    motion_matrix = np.zeros((raw_images.shape[0], motion_matrix_shapes[0][0], motion_matrix_shapes[0][1],  3), dtype=np.int32)

    for level in range(pyramid_levels):
        if level != 0:
            motion_matrix = upsample_image(motion_matrix, motion_matrix_shapes[level][0], motion_matrix_shapes[level][1]) * downsample_factor
        motion_matrix = sparse_align_level_images(level, np.array(pyramid_images[level]), motion_matrix, reference_image_index, block_size=8 if level < 2 else 16, search_window=4)
    return motion_matrix

@njit(parallel=True)
def sparse_align_level_images(level, pyramid_level_images, motion_matrix, reference_image_index, block_size=16, search_window=4):
    level_reference_image = pyramid_level_images[reference_image_index]
    x_max, y_max = level_reference_image.shape
    for i in prange(motion_matrix.shape[0]):
        for x_motion_ptr in prange(0, x_max // block_size):
            for y_motion_ptr in prange(0, y_max // block_size):
                x = x_motion_ptr * block_size
                y = y_motion_ptr * block_size
                # Initialize the best match and the best match error
                best_match = np.zeros(3, dtype=np.int64)
                best_match_error = np.inf
                ...
                reference_block = level_reference_image[x:end_x, y:end_y]
                image_offset = motion_matrix[i]
                offset = image_offset[x_motion_ptr, y_motion_ptr]
                ...
                for x_offset in range(-search_window + offset[0], search_window  + offset[0]):
                    for y_offset in range(-search_window + offset[1], search_window + offset[1]):
                        ...
                        if block_error < best_match_error:
                            best_match_error = block_error
                            best_match = np.array([x_offset, y_offset, 0], dtype=np.int64)
                motion_matrix[i][x_motion_ptr, y_motion_ptr] = best_match
    return motion_matrix